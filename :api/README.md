...
index.ts:  The Brain. Contains the core Capture Logic. It receives incoming scans, assigns idempotency keys, and saves them to the database.prisma/ FolderThe Blueprint Room. Holds the database schema definitions.schema.prismaThe Structural Map. Defines the "Airing Object" (ID, timestamp, payload) and ensures the database enforces the unique identity of every scan.
